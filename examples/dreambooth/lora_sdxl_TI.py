...
def create_network_from_weights(multiplier, file, text_encoder, weights_sd=None, for_inference=False, **kwargs):
    ...
    network = LoRANetwork(
        text_encoder, multiplier=multiplier, modules_dim=modules_dim, modules_alpha=modules_alpha, module_class=module_class
    )
...
class LoRANetwork(torch.nn.Module):
    ...
    def __init__(
        ...
        for i, text_encoder in enumerate(text_encoders):
            ...
            text_encoder_loras, skipped = create_modules(index, text_encoder, LoRANetwork.TEXT_ENCODER_TARGET_REPLACE_MODULE)
    ...
    def apply_to(self, text_encoder, apply_text_encoder=True):
    ...
    def merge_to(self, text_encoder, weights_sd, dtype, device):
        ...
        for lora in self.text_encoder_loras:
            ...
            lora.merge_to(sd_for_lora, torch.float32, 'cuda')
    ...
    def prepare_optimizer_params(self, text_encoder_lr):
    ...
    def prepare_grad_etc(self, text_encoder):
        self.requires_grad_(True)

    def on_epoch_start(self, text_encoder):
        self.train()
